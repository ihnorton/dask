...
def from_tiledb(uri, attribute=None, chunks=None,
    ...
    def tiledb_to_chunks(array):
        ...
        TileDB tile extends corresponds to form 2 of dask chunk
        specifiers:
    ...
    if tdb.schema.sparse:
        raise Exception("Sparse TileDB arrays are not supported")
    ...
    chunks = tiledb_to_chunks(tdb)

    assert(len(chunks) == tdb.schema.ndim)

    return from_array(tdb, chunks, name='tiledb-%s' % uri)
...
def to_tiledb(darray, uri, compute=True, return_stored=False,
    ...
    if not isinstance(darray, Array):
        raise TypeError("to_tiledb first argument must be a dask array")
